...
// Get the ID Peminjaman Ruangan from the URL
$idPeminjamanRuangan = isset($_GET['id']) ? htmlspecialchars($_GET['id']) : '';

// If ID is not present, redirect or show an error
if (empty($idPeminjamanRuangan)) {
    // Optionally, set a session message for an error
    // $_SESSION['message'] = ['type' => 'danger', 'text' => 'ID Peminjaman tidak valid.'];
    // header('Location: riwayatPeminjamanAdmin.php');
    // exit();
    // For now, just die with a message if ID is missing, for simplicity during development
...
<head>
  ...
  <title>Alasan Penolakan Peminjaman - Admin - Sistem Pengelolaan Laboratorium</title>
...
<body class="bg-light">
    ...
    <header class="d-flex align-items-center justify-content-between px-3 px-md-5 py-3">
      <div class="d-flex align-items-center">
        ...
        <div class="d-none d-md-block ps-3 ps-md-4" style="margin-left: 5vw;">
          ...
          <span class="fw-normal fs-6">Admin (PIC)</span>
        </div>
      </div>
      <div class="d-flex align-items-center">
        ...
        <ul class="nav nav-pills flex-column mb-auto">
          ...
          <li class="nav-item mb-2">
            ...
            <div class="collapse ps-4" id="asetSubmenu">
              ...
              <a href="#" class="nav-link">Ruangan</a> <!-- Placeholder -->
            </div>
          </li>
          <li class="nav-item mb-2">
            <a class="nav-link d-flex justify-content-between align-items-center" data-bs-toggle="collapse" href="#akunSubmenu" role="button" aria-expanded="false" aria-controls="akunSubmenu">
              <span class="d-flex align-items-center"><img src="icon/iconamoon-profile-fill0.svg" style="vertical-align: middle; margin-right: 0.5rem;">Manajemen Akun</span>
              <i class="bi bi-chevron-down transition-chevron ps-3"></i>
            </a>
            <div class="collapse ps-4" id="akunSubmenu">
              <a href="#" class="nav-link">Mahasiswa</a> <!-- Placeholder -->
              <a href="#" class="nav-link">Karyawan</a> <!-- Placeholder -->
            </div>
          </li>
          <li class="nav-item mb-2">
            <a class="nav-link d-flex justify-content-between align-items-center parent-active" data-bs-toggle="collapse" href="#pinjamSubmenu" role="button" aria-expanded="true" aria-controls="pinjamSubmenu">
          ...
          <li class="nav-item mb-2">
            <a href="#" class="nav-link"><img src="icon/graph-report0.png" class="sidebar-icon-report">Laporan</a> <!-- Placeholder -->
          </li>
        ...
        <div class="offcanvas-body p-0">
          <nav class="sidebar flex-column p-4 h-100">
            <ul class="nav nav-pills flex-column mb-auto">
              ...
              <li class="nav-item mb-2">
                <a class="nav-link d-flex justify-content-between align-items-center parent-active" data-bs-toggle="collapse" href="#pinjamSubmenuMobile" role="button" aria-expanded="true" aria-controls="pinjamSubmenuMobile">
                  <span><img src="icon/ic-twotone-sync-alt0.svg">Peminjaman</span>
                  <i class="bi bi-chevron-down transition-chevron ps-3"></i>
                </a>
      ...
      <main class="col bg-white px-3 px-md-4 py-3 position-relative">
        <div class="mb-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item"><a href="detailRiwayatRuanganAdmin.php?id=<?php echo $idPeminjamanRuangan; ?>">Detail Peminjaman</a></li>
              <li class="breadcrumb-item active" aria-current="page">Alasan Penolakan</li>
            </ol>
          </nav>
        </div>
        ...
        <div class="card border border-dark mt-4">
          <div class="card-header bg-white border-bottom border-dark">
            <span class="fw-semibold">Form Alasan Penolakan Peminjaman Ruangan</span>
          </div>
          <div class="card-body">
            <form method="POST" action="process_penolakan.php" onsubmit="return validatePenolakanForm();">
              <input type="hidden" name="idPeminjamanRuangan" value="<?php echo $idPeminjamanRuangan; ?>">
              
              <div class="mb-3">
                <label for="idPeminjamanDisplay" class="form-label">ID Peminjaman Ruangan</label>
                <input type="text" readonly class="form-control" id="idPeminjamanDisplay" value="<?php echo $idPeminjamanRuangan; ?>">
              </div>
              
              <div class="mb-3">
                <label for="alasanPenolakan" class="form-label">Alasan Penolakan</label> <span id="alasanPenolakanError" style="color: red; font-size: 0.9em;"></span>
                <textarea class="form-control" id="alasanPenolakan" name="alasanPenolakan" rows="4" placeholder="Masukkan alasan penolakan..."></textarea>
              </div>
              
              <div class="mt-4 d-flex justify-content-between">
                <a href="detailRiwayatRuanganAdmin.php?id=<?php echo $idPeminjamanRuangan; ?>" class="btn btn-secondary">Kembali</a>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary ps-4 pe-4" onclick="window.location.href='logout.php';">Ya</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    function validatePenolakanForm() {
      const alasanTextarea = document.getElementById('alasanPenolakan');
      const alasanErrorSpan = document.getElementById('alasanPenolakanError');
      
      if (alasanTextarea.value.trim() === '') {
        alasanErrorSpan.textContent = 'harus diisi';
        alasanTextarea.focus(); // Optional: focus the textarea
        return false; // Prevent form submission
      } else {
        alasanErrorSpan.textContent = ''; // Clear error message
