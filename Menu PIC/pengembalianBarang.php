<?php
include '../koneksi.php';
$query = "SELECT idPeminjamanBrg, catatanPengembalianBarang, kondisiBrg FROM Pengembalian_Barang";
$result = sqlsrv_query($conn, $query);
$currentPage = basename($_SERVER['PHP_SELF']); // Determine the current page

$idPeminjamanBrg = isset($_GET['id']) ? $_GET['id'] : '';
$data = []; // Inisialisasi array data

if (!empty($idPeminjamanBrg)) {
    $query = "SELECT pb.*, pm.jumlahBrg 
              FROM Pengembalian_Barang pb 
              JOIN Peminjaman_Barang pm ON pb.idPeminjamanBrg = pm.idPeminjamanBrg 
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Validasi sederhana
    if (!empty($jumlahBarang) && !empty($kondisiBrg) && $kondisiBrg != 'Pilih Kondisi Barang') {
        $query = "UPDATE Pengembalian_Barang 
                  SET jumlahKembali = ?, catatanPengembalianBarang = ?, kondisiBrg = ?
                  WHERE idPeminjamanBrg = ?";
        $params = array($jumlahBarang, $catatan, $kondisiBrg, $idPeminjamanBrg);
    ...
    //     }
    // } else {
    //     echo '<div class="alert alert-warning">Mohon lengkapi semua isian dengan benar.</div>';
    }
}
    ...
    <div class="container-fluid min-vh-100 d-flex flex-column p-0">
                            ...
                            <div class="card border border-dark">
                                ...
                                <div class="card-body">
                                    <form method="POST">
                                        ...
                                        <div class="mb-2 row">
                                            <div class="col-md-3">
                                                <label for="jumlahPinjam" class="form-label">Jumlah Peminjaman</label>
                                                <input type="text" class="form-control" id="jumlahPinjam" name="jumlahPinjam" value="<?= isset($data['jumlahBrg']) ? htmlspecialchars($data['jumlahBrg']) : '' ?>" disabled>
                                            </div>
                                            <div class="col-md-4">
                                                <label for="jumlahBarang" class="form-label w-100 text-center">Jumlah Pengembalian
                                                ...
                                                <div class="input-group mx-auto" style="max-width: 140px;">
                                                    ...
                                                    <input class="form-control text-center" id="jumlahBarang" name="jumlahBarang" value="0" min="0" required style="max-width: 70px;">
                                                    <button class="btn btn-outline-secondary" type="button" onclick="changeStok(1)">+</button>
                                                </div>
                                            </div> 
                                            <div class="col-md-5">
                                        ...
                                        <div class="mb-2">
                                            <label for="catatan" class="form-label">Catatan Pengembalian
                                                <span id="catatanError" class="text-danger small mt-1" style="font: size 0.95em;display:none;">*Harus Diisi</span>
                                            </label>
                                            <textarea type="text" class="form-control" id="catatan" name="catatan" rows="3" style="resize: none;"><?= isset($data['catatanPengembalianBarang']) ? htmlspecialchars($data['catatanPengembalianBarang']) : '' ?></textarea> 
                                        </div>
        ...
        <script>
            function changeStok(val) {
                var stokInput = document.getElementById('jumlahBarang');
       ...
       <script>
            document.querySelector('form').addEventListener('submit', function(e) {
                ...
                if (!jumlahError) {
                    jumlahError = document.createElement('div');
                    jumlahError.id = 'jumlahError';
                    jumlahError.className = 'text-danger small mt-1';
                    jumlah.parentNode.appendChild(jumlahError);
                }
                if (jumlah.value.trim() === '' || jumlah.value.trim() === '0') {
                    jumlahError.textContent = 'Jumlah pengembalian harus diisi dan lebih dari 0.';
                    valid = false;
                } else {
                ...
                // Kondisi Barang
                const kondisi = document.getElementById('txtKondisi');
                let kondisiError = document.getElementById('kondisiError');
                if (!kondisiError) {
                    kondisiError = document.createElement('div');
                    kondisiError.id = 'kondisiError';
                    kondisiError.className = 'text-danger small mt-1';
                    kondisi.parentNode.appendChild(kondisiError);
                }
                if (kondisi.value === 'Pilih Kondisi Barang') {
                    kondisiError.textContent = 'Kondisi barang harus dipilih.';
                    valid = false;
                } else {
                ...
                if (!catatanError) {
                    catatanError = document.createElement('div');
                    catatanError.id = 'catatanError';
                    catatanError.className = 'text-danger small mt-1';
                    catatan.parentNode.appendChild(catatanError);
                }
                if (catatan.value.trim() === '') {
                    catatanError.textContent = 'Catatan pengembalian harus diisi.';
                    valid = false;
                } else {
