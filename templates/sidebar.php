...
// Fungsi untuk render menu sidebar (agar tidak duplikat)
function renderSidebarMenu($role, $isPeminjam, $currentPage)
{
    ...
    ?>
        ...
        <li class="nav-item mb-2">
            <a href="<?= BASE_URL ?>/Menu PIC/laporan.php" class="nav-link"><img src="<?= BASE_URL ?>/icon/graph-report0.png" class="sidebar-icon sidebar-icon-report">Laporan</a>
        </li>
    <?php
    elseif ($isPeminjam) :
    ?>
...
@media (max-width: 991.98px) {
    ...
    .offcanvas .nav-link.active, .offcanvas .nav-link:active, .offcanvas .nav-link:focus {
        background: #2563eb;
        color: #fff !important;
    }
